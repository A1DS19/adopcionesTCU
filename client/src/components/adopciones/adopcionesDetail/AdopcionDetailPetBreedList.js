...
const AdopcionDetailPetBreedList = ({ selectedPet, petsData, }) => {
    ...
    const renderCurrentBreedPets = currentBreedPets
        .map((pet) => (<AdopcionDetailListItem_1.AdopcionDetailListItem key={pet._id} selectedBreedPet={pet}/>))
        .slice(0, 3);
    return (<react_1.Fragment>
      {currentBreedPets.length !== 0 && (<semantic_ui_react_1.Segment>
          <semantic_ui_react_1.Header textAlign='center' as='h1' content={`Mas rescates de raza ${selectedPet === null || selectedPet === void 0 ? void 0 : selectedPet.breed}`}/>
          <semantic_ui_react_1.Card.Group itemsPerRow={3} doubling stackable>
